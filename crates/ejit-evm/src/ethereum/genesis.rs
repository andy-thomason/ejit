...
use crate::json::{JsonDecode, JsonError, ObjectParser};
...
impl<'de> JsonDecode<'de> for Genesis {
    fn decode_json(&mut self, buffer: & mut &'de [u8]) -> Result<(), JsonError> {
        ...
        loop {
            match p.next_key()? {
                Some(k) if k == "nonce" => self.header.nonce.decode_json(p.buffer)?,
                Some(k) if k == "timestamp" => self.header.timestamp.decode_json(p.buffer)?,
                Some(k) if k == "extraData" => self.header.extra_data.decode_json(p.buffer)?,
                Some(k) if k == "gasLimit" => self.header.gas_limit.decode_json(p.buffer)?,
                Some(k) if k == "difficulty" => self.header.difficulty.decode_json(p.buffer)?,
                Some(k) if k == "mixHash" => self.header.prev_randao.decode_json(p.buffer)?,
                Some(k) if k == "coinbase" => self.header.coinbase.decode_json(p.buffer)?,
                Some(k) if k == "stateRoot" => self.header.state_root.decode_json(p.buffer)?,
                Some(k) if k == "alloc" => self.alloc.decode_json(p.buffer)?,
                Some(k) if k == "number" => self.header.number.decode_json(p.buffer)?,
                Some(k) if k == "gasUsed" => self.header.gas_used.decode_json(p.buffer)?,
                Some(k) if k == "parentHash" => self.header.parent_hash.decode_json(p.buffer)?,
                None => return Ok(()),
